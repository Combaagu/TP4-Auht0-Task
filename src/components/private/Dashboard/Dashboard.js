...
import { Button, Layout, Typography, Pagination, Card, Spin } from 'antd';
...
const Dashboard = () => {
  ...
  const [loadingTasks, setLoadingTasks] = useState(true); // Estado para manejar la carga de tareas

  useEffect(() => {
    const fetchTasks = async () => {
      try {
        setLoadingTasks(true); // Inicia la carga
        const params = {
          page: page - 1,
          perPage: perPage,
          filter: {},
        ...
        setLoadingTasks(false); // Finaliza la carga
      }
    };
